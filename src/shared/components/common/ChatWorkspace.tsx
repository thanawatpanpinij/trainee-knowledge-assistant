...
interface ChatWorkspaceProps {
  chatId?: string // รับมาจาก Dynamic Route หรือไม่ส่งมาเลย (แชตใหม่)
}
...
export default function ChatPage({
    ...
    transport: new DefaultChatTransport({
      fetch: async (url, options) => {
        ...
        try {
          ...
          if (returnedChatId && !initialChatId) {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        // อัปเดต Path ให้ตรงกับ API ใหม่ของเรา
        const res = await fetch(`/api/chat/history?chatId=${initialChatId}`)
        if (res.ok) {
